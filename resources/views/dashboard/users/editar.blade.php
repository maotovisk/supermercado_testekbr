    ...
    <x-app-layout>
        <x-slot name="header">
            <div class="flex justify-between items-center">
                <p class="text-xl text-gray-800 font-semibold">{{ __('Editar Usuário') }}</p>
                        ...
                        <form method="POST" action="{{ route('users.atualizar', $user->id) }}">
                            ...
                            <div class="block mt-4">
                                <label for="is_admin" class="inline-flex items-center">
                                    <input {{$user->is_admin ? "checked" :"" }} value="{{$user->is_admin ? 'on' : 'off'}}" id="is_admin" type="checkbox" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" name="admin">
