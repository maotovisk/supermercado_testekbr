        ...
        <x-slot name="header">
            <div class="flex justify-between items-center">
                ...
                @if (Auth::user()->is_admin)
                    <button onclick="window.location.href='{{route('users.novo')}}'" class="bg-gray-800 hover:bg-gray-900 text-white text-xs px-6 py-2 rounded-lg border-0">Adicionar
                        Novo</button>
                @endif
            </div>
        </x-slot>
                            ...
                            <table class="overflow-x-auto w-full bg-white divide-y divide-gray-200">
                                <thead class="bg-gray-50 text-gray-500 text-sm">
                                    <tr class="divide-x divide-gray-300">
                                        ...
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Email
                                        </th>
                                        @if (Auth::user()->is_admin)
                                ...
                                <tbody class="text-gray-500 text-xs divide-y divide-gray-200">
                                    @foreach ($users as $user)
                                        <tr>
                                            ...
                                            @if (Auth::user()->is_admin)
                                                <td class="py-2 px-3">
                                                    <span class="bg-indigo-200 text-indigo-500 text-xs font-semibold rounded-md py-1 px-2">{{ $user->is_admin ? 'Sim' : 'Não' }}</span>
                                                </td>
                                                <td class="py-2">
                                                    <div
                                                        ...
                                                        <div class="divide-x-2 border border-gray-300 shadow-sm rounded-md">
                                                            <button onclick="window.location.href='{{route('users.editar', $user->id)}}'" class="p-2">
